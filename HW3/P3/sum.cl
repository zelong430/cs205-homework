                            ...
                            long N)
{
    ...
    for (long i=0; global_id + i * global_size < N; i++) {
    ...
    for (uint s = group_size/2; s>0 ; s >>= 1) { // YOUR CODE HERE
        if (local_id < s){
                          ...
                          long N)
{
    ...
    long global_id = get_global_id(0);
    long group_size = get_local_size(0);
    long k = ceil((float)N / get_global_size(0));
    ...
    for (long idx = k * global_id; idx <= k*(global_id + 1) - 1 ; idx++) { 
        if (idx <= N){
            sum += x[idx];
        } 
    }
